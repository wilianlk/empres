...
CREATE TABLE `prod_talla` (
  ...
  `nombre_ing` varchar(15) DEFAULT NULL,
  ...
  CONSTRAINT `prod_talla_created_by_sf_guard_user_id` FOREIGN KEY (`created_by`) REFERENCES `users` (`id_user`) ON DELETE CASCADE,
  CONSTRAINT `prod_talla_updated_by_sf_guard_user_id` FOREIGN KEY (`updated_by`) REFERENCES `users` (`id_user`) ON DELETE CASCADE
